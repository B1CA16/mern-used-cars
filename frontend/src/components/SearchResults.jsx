...
const SearchResults = () => {
    ...
    useEffect(() => {
        ...
        if (urlFromYear) {
            filtered = filtered.filter(
                (car) => car.year >= parseInt(urlFromYear)
            );
        }
        if (urlUntilYear) {
            filtered = filtered.filter(
                (car) => car.year <= parseInt(urlUntilYear)
            );
        }
        if (urlMinPrice) {
            filtered = filtered.filter(
                (car) => car.price >= parseFloat(urlMinPrice)
            );
        }
        if (urlMaxPrice) {
            filtered = filtered.filter(
                (car) => car.price <= parseFloat(urlMaxPrice)
            );
        }
        if (urlMileageFrom) {
            filtered = filtered.filter(
                (car) => car.km >= parseFloat(urlMileageFrom)
            );
        }
        if (urlMileageTo) {
            filtered = filtered.filter(
                (car) => car.km <= parseFloat(urlMileageTo)
            );
        }
        if (urlSegment) {
            filtered = filtered.filter(
        ...
        <div className="most-popular-section px-2 sm:px-12 py-12">
            {loading ? (
                ...
                <div className="w-11/12 xl:w-3/4 mx-auto">
                    <div
                        ref={resultsRef}
                        className="flex justify-between px-4 items-center mb-6"
                    >
                        <p className="text-lg">
                            Showing <strong>{currentCars.length}</strong> of{" "}
                            <strong>{sortedCars.length}</strong>{" "}
